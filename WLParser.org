...
# Create the .texi file from the .org source
texi: WLParser.texi
WLParser.texi: WLParser.org
	emacs WLParser.org --batch -f org-texinfo-export-to-texinfo --kill

# Create the .info file from the .org source
info: WLParser.org
	emacs WLParser.org --batch -f org-texinfo-export-to-info --kill

# Create a docs/ dir if necessary; move .info file into it
install-info: $(DOCS)/WLParser.info

# Create a docs/ dir if necessary and install the .info file into the docs/ dir
$(DOCS)/WLParser.info: info | docs-dir
	mv -v WLParser.info $(dir $@)

# Install a pdf file into a docs/ dir
install-pdf: $(DOCS)/WLParser.pdf
