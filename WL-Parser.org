...
** Implementation of the WLReader Module
...
#+begin_src js -n :tangle src/wl_checks.js
  ...
  class WLChecks extends EventEmitter {
      constructor(year, rec_sep) {
          super();
          this._checks = [];
          const wltype = new WLType(year, rec_sep);
          wltype.on('TRUST', trust_record => {
              this.emit('check', trust_record);
              this._checks.push(trust_record);
              /* parse the trust record for check info */
          }).on('parsed', () => {
...
#+begin_src js -n :tangle __tests__/wl_checks.test.js
  ...
  beforeAll(done => {
      checks = new WLChecks(YEAR, REC_SEP);
      done();
  });

  describe('WLChecks', () => {
      it('checks is a WLChecks', () => {
          expect(checks).toBeInstanceOf(WLChecks);
      });
  });

  describe('An instance of WLChecks', () => {
      test('checks is an Array', () => {
          expect(checks.all).toBeInstanceOf(Array);
      });
  });
#+end_src
